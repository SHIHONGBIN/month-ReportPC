function canvasHanle(){
  ...
  var maxWidthHeight = {
    width: windowObj.width-200,
    height: windowObj.height - 200
  }
  ...
  function objInit(){
    objSanJiao.map((item, index)=>{
      let line = Math.round(Math.random()*100) + 60
      let startP = {
        left:eval(Math.round(Math.random()*maxWidthHeight.width)),
        top:eval(Math.round(Math.random()*maxWidthHeight.height+50)),
      }
